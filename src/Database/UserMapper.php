...
class UserMapper
{
    ...
    {
        try {
            $sql = 'SELECT `name` FROM `peshkariki_users` WHERE `id` = :id';
    ...
    {
        try {
            $sql = 'SELECT `id`, `name` FROM `peshkariki_users` WHERE `name` = :name';
            $stmt = $this->pdo->prepare($sql);
    ...
    {
        try {
            $sql = 'SELECT `id` FROM `peshkariki_users` WHERE `name` = :name';
            $stmt = $this->pdo->prepare($sql);
    ...
    {
        try {
            $sql = 'INSERT INTO `peshkariki_users`(`name`, `hash`) VALUES (:name, :hash)';
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':name', $name);
    ...
    {
        try {
            $sql = 'SELECT `hash` FROM `peshkariki_users` WHERE `id` = :id';
            $stmt = $this->pdo->prepare($sql);
    ...
    {
        try {
            $sql = 'SELECT `hash` FROM `peshkariki_users` WHERE `name` = :name';
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':name', $username);
            $result = $stmt->execute();
            if ($result !== false) {
    ...
    {
        try {
            $sql = 'UPDATE `peshkariki_users` SET `hash` = :hash WHERE `name` = :name';
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':name', $username);
    ...
    {
        try {
            $sql = 'DELETE FROM `peshkariki_users` WHERE `id` = :id';
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindParam(':id', $userID, \PDO::PARAM_INT);
            $result = $stmt->execute();
        } catch (\PDOException $e) {
